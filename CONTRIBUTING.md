# Contributing to Milpa OAuth Providers

Thanks for your interest in contributing! Milpa OAuth Providers is the OAuth 2.0 /
social-login provider protocol layer of the Milpa framework — typed contracts and
curl-based implementations for Google, GitHub, GitLab, Facebook, Apple, Twitch and
the Telegram Login Widget, plus immutable UserInfo DTOs. It builds authorization
URLs, exchanges codes for tokens, and normalizes provider responses into typed
DTOs — nothing else.
...
- **Zero dependencies, by design.** Milpa OAuth Providers has no runtime dependency
  on `milpa/core` or any other Milpa package — just PHP's `curl` and `openssl`
  extensions. Do not introduce one. Do not add a Doctrine, HTTP-client, or session
  dependency, and do not push storage/persistence concerns (tokens, CSRF `state`,
  sessions) into this package — generating, storing, and verifying the `state`
  parameter is deliberately left to the caller; each provider only transports it.
